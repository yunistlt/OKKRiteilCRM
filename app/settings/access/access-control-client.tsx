...
type Props = {
    ...
    initialInvitations: AccessInvitation[];
    invitationsTableReady: boolean;
};

export default function AccessControlClient({ initialAccounts, initialManagers, initialRouteRules, routeRulesTableReady, initialRoleCapabilities, roleCapabilitiesTableReady, initialInvitations, invitationsTableReady }: Props) {
    ...
    const handleCreateInvitationForAccount = (account: AccessAccount) => {
        ...
        startTransition(async () => {
            ...
            if (!result.success || !result.invitation) {
                if (result.errorType === 'TABLE_MISSING') setShowSqlGuide(true);
                setMessage(result.message || 'Не удалось создать ссылку-приглашение.');
                return;
            }
    ...
    const handleRevokeInvitation = (id: string) => {
        setMessage('');
        startTransition(async () => {
            const result = await revokeAccessInvitation(id);
            if (!result.success) {
                setMessage(result.message || 'Не удалось отозвать ссылку.');
                return;
            }
            setInvitations((current) => current.map((item) => item.id === id ? { ...item, revoked: true } : item));
            setMessage(result.message || 'Ссылка-приглашение отозвана.');
        });
    };
