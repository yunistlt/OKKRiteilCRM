                    ...
                    <table className="w-full border-collapse text-left text-[13px]">
                        <thead className="sticky top-0 z-10">
                            <tr className="bg-slate-50 text-[10px] uppercase tracking-wider text-slate-400">
                                <th className="border-b border-slate-200 px-3 py-2 font-semibold">Тип / Время</th>
                                <th className="border-b border-slate-200 px-3 py-2 font-semibold">От кого</th>
                                <th className="border-b border-slate-200 px-3 py-2 font-semibold">Тема</th>
                                <th className="border-b border-slate-200 px-3 py-2 font-semibold">Письмо</th>
                                <th className="border-b border-slate-200 px-3 py-2 font-semibold">Менеджер</th>
                                <th className="border-b border-slate-200 px-3 py-2 font-semibold">Заказ</th>
                                <th className="border-b border-slate-200 px-3 py-2 font-semibold">Переслано</th>
                                <th className="border-b border-slate-200 px-3 py-2 font-semibold">Вывод</th>
                            </tr>
                        </thead>
                        <tbody>
                            {(recent || []).map((r: any) => {
                                ...
                                return (
                                    <tr key={r.id} className="border-b border-slate-100 align-top odd:bg-white even:bg-slate-50/40 hover:bg-sky-50/50">
                                        ...
                                        <td className="px-3 py-2 text-slate-700">{r.assigned_manager_id ? (names[Number(r.assigned_manager_id)] || r.assigned_manager_id) : <span className="text-slate-300">—</span>}</td>
                                        <td className="px-3 py-2 whitespace-nowrap">
                                            {r.created_crm_order_id && orderUrl(r.created_crm_order_id) ? (
                                                <a href={orderUrl(r.created_crm_order_id) as string} target="_blank" rel="noopener noreferrer"
                                                ...
                                                <span className="border border-rose-300 bg-rose-50 px-1.5 py-0.5 text-[10px] font-bold text-rose-900">ошибка создания</span>
                                            ) : (
                                                <span className="text-slate-300">—</span>
                                            )}
                                        </td>
                                        <td className="px-3 py-2 whitespace-nowrap">
                                            {r.forwarded_to ? (
                                                ...
                                                r.status === 'needs_review' ? (
                                                    <span className="border border-amber-300 bg-amber-50 px-1.5 py-0.5 text-[10px] font-bold text-amber-900">адрес не настроен</span>
                                                ) : r.status === 'error' ? (
                                                    <span className="border border-rose-300 bg-rose-50 px-1.5 py-0.5 text-[10px] font-bold text-rose-900">ошибка пересылки</span>
                                                ) : (
                                        ...
                                        <td className="px-3 py-2 max-w-[340px] text-xs text-slate-500">{r.reasoning || <span className="text-slate-300">—</span>}</td>
                                    </tr>
                                );
                            })}
                            {(recent || []).length === 0 ? (
