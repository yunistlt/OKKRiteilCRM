...
async function loadOrder(orderId: number, userRole: string, retailCrmManagerId: number | null): Promise<ConsultantOrder> {
    ...
    const [{ data: orderRow, error: orderError }, { data: scoreRow, error: scoreError }] = await Promise.all([
        supabase.from('orders').select('order_id, status, manager_id').eq('order_id', orderId).maybeSingle(),
        supabase.from('okk_order_scores').select('*').eq('order_id', orderId).maybeSingle(),
    ]);
    ...
    const [{ data: managerData }, { data: statusData }] = await Promise.all([
        managerId ? supabase.from('managers').select('first_name, last_name').eq('id', managerId).maybeSingle() : Promise.resolve({ data: null }),
        orderRow?.status ? supabase.from('statuses').select('name, color').eq('code', orderRow.status).maybeSingle() : Promise.resolve({ data: null }),
    ]);
...
async function loadEvidence(orderId: number): Promise<OrderEvidence> {
    ...
    const [
        ...
        supabase.from('raw_order_events').select('event_id', { count: 'exact', head: true }).eq('retailcrm_order_id', orderId).ilike('event_type', '%comment%'),
        supabase.from('raw_order_events').select('event_id', { count: 'exact', head: true }).eq('retailcrm_order_id', orderId).ilike('event_type', '%email%'),
        supabase.from('call_order_matches').select('raw_telphin_calls(direction, transcript, started_at, duration_sec)').eq('retailcrm_order_id', orderId),
        supabase.from('order_history_log').select('field, occurred_at, old_value, new_value').eq('retailcrm_order_id', orderId).order('occurred_at', { ascending: false }).limit(10),
        supabase.from('orders').select('raw_payload').eq('order_id', orderId).maybeSingle(),
    ]);
...
export async function GET(req: Request) {
    try {
        ...
        const rawOrder = await loadOrder(orderId, session.user.role || 'admin', retailCrmManagerId);
        const rawEvidence = await loadEvidence(orderId);
