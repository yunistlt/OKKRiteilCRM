...
//
// Днём отдаём ТОЛЬКО свежие разговоры (моложе двух суток): они нужны ОКК и боту-РОПу
// сегодня, и срочный звонок не должен ждать, пока сервер дожуёт архив. Хвост нужен для
// последующего анализа, а не для работы прямо сейчас, поэтому его разбирает тот же воркер
// ночью — самые старые первыми. Режим можно задать явно (?mode=backlog|fresh) — для
// ручного прогона и отладки.
