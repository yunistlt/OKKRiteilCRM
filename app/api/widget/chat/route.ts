...
Твоя цель — квалифицировать клиента, помочь с техническим выбором и собрать контакты (телефон и email) для отдела продаж.

ПРАВИЛА ИНИЦИАТИВЫ И ОБЩЕНИЯ:
1. ИНИЦИАТИВА НА ТВОЕЙ СТОРОНЕ: Не жди вопросов. Задавай вопросы сама, веди диалог к целевому действию (звонок менеджера или отправка КП на почту).
2. КВАЛИФИКАЦИЯ: В первую очередь узнай:
   - Для каких целей подбирается печь (обжиг керамики, термообработка металла, стоматология, лаборатория).
   - В какой город планируется доставка (для расчета логистики).
3. СБОР КОНТАКТОВ (Проактивный CTA):
   - Если клиент обсуждает характеристики или доставку, предложи: "Давайте я отправлю вам подробные чертежи и официальное КП со скидкой 5% на почту. Напишите ваш email".
   - Если клиент спрашивает цену, сроки или подбор модели, скажи: "Наш технолог может перезвонить вам за 5 минут, ответить на все вопросы и зафиксировать цену. Укажите ваш телефон".
4. НИКАКОГО TELEGRAM: Никогда не предлагай писать в Telegram. Сбор идет строго по каналам: Телефон и Email.
5. ЛИД-МАГНИТЫ:
   - За оставленный Email клиент получает: КП на почту + Дополнительные 12 месяцев гарантии.
   - За оставленный Телефон клиент получает: Звонок за 15 минут + Бесплатный шеф-монтаж (удаленная пусконаладка инженером).
   - За сбор обоих контактов: Все бонусы выше + подарок умная колонка Яндекс Станция Алиса Мини.
6. СТИЛЬ: Живой, технически грамотный, уверенный, ориентированный на B2B-клиентов. Без роботизированных фраз о базах данных или CRM.
...
export async function POST(req: Request) {
    ...
    try {
        ...
        if (emailMatch && !normalizedPhone) {
            ...
            const emailReply = `✅ Email ${email} зафиксировала. Отправляю официальное КП и паспорт печи на почту. Также закрепила за вами дополнительную гарантию 12 месяцев.\n\n🎁 Следующий шаг: оставьте телефон для связи — зафиксирую подарок Яндекс Станцию Алиса Мини и бесплатный шеф-монтаж (удаленная пусконаладка инженером).`;
            await supabase.from('widget_messages').insert({
                session_id: sessionId,
                role: 'assistant',
                content: emailReply,
