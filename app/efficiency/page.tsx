    ...
    return (
        <div className="p-8 max-w-7xl mx-auto font-sans bg-gray-50 min-h-screen">
            <div className="flex justify-between items-center mb-10">
                <div>
                    <h1 className="text-4xl font-black text-gray-900 tracking-tight">Эффективность Менеджеров</h1>
                    <p className="text-gray-500 mt-2 text-lg">Анализ рабочего времени и производительности</p>
                </div>
            </div>
            ...
            {loading ? (
                ...
                <div className="p-20 text-center bg-white rounded-3xl border-2 border-dashed border-gray-100">
                    <h3 className="text-xl font-black text-gray-900">Данные не найдены</h3>
                </div>
            ) : (
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-gray-50/50 text-gray-400 text-[10px] uppercase tracking-[0.2em] font-black border-b border-gray-100">
                                <th className="p-6">Менеджер</th>
                                <th className="p-6">Время в работе (Σ)</th>
                                <th className="p-6">Заказов обработано</th>
                                <th className="p-6">Среднее время / заказ</th>
                                ...
                                <tr key={row.manager_id} className="hover:bg-blue-50/20 transition-all duration-200 group">
                                    <td className="p-6">
                                        <Link
                                            ...
                                            className="text-lg font-black text-gray-900 hover:text-blue-600 transition-colors uppercase tracking-tight"
                                        >
                                            {row.manager_name}
                                        </Link>
                                    </td>
                                    <td className="p-6">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-[10px] font-bold text-gray-400 uppercase">({row.total_minutes} мин)</span>
                                        </div>
                                    </td>
                                    <td className="p-6 text-gray-600 font-bold tabular-nums">
                                        {row.processed_orders}
                                    </td>
                                    <td className="p-6">
                                        <div className="inline-flex items-center px-3 py-1 bg-green-50 text-green-700 rounded-lg text-xs font-black uppercase tracking-wider">
                                    ...
                                    <td className="p-6 text-right">
                                        <Link
                                            href={`/analytics/managers/${row.manager_id}`}
                                            className="px-6 py-2 bg-gray-50 text-gray-900 rounded-xl hover:bg-gray-900 hover:text-white transition-all font-black text-[10px] uppercase tracking-wider border border-gray-100"
                                        >
                                            Профиль
                                        </Link>
                                    </td>
