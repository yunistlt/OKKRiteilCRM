...
const RETAILCRM_SITE = process.env.RETAILCRM_SITE || 'zmktlt-ru';

async function testCrmUpdate(customerId) {
    console.log(`Testing RetailCRM update (UrlEncoded JSON / SUCCESS version) for Customer ID: ${customerId}`);
    
    const customerData = {
        customFields: {
            ai_reactivation_text: 'TEST MESSAGE FINAL SUCCESS: ' + new Date().toISOString()
        }
    };
