...
- [x] Зафиксировать ограничение API RetailCRM v5: использовать limit только 20, 50 или 100.
- [x] Для штатного дельта-поллинга использовать limit=50 как основной режим.
- [x] Использовать limit=100 только в catch-up режиме, если backlog уже накопился.
- [x] Не делать бесконечный цикл страниц в одном запуске; ограничить штатный прогон 1-2 страницами за итерацию.
- [x] Для catch-up режима ограничить прогон 5-10 страницами за итерацию с явным time budget.
- [x] Поставить базовую частоту дельта-поллинга заказов: 1 раз в 60 секунд в рабочее время.
- [x] Поставить пониженную частоту дельта-поллинга заказов: 1 раз в 180 секунд в ночные часы.
...
- [x] Использовать count=100 только для catch-up и backfill; в штатном режиме не перегружать обработку лишними страницами.
...
- [x] Ввести отдельный backlog-recovery job, который медленно дочищает пропуски без давления на основной поток.
...
- [x] На webhook call_end сразу создавать matching job, если order ещё не найден.
...
- [x] Убрать зависимость пользовательских экранов от ручного refresh quality analytics.
...
- [x] Несcheduled admin sync endpoints `sync/dictionaries`, `sync/statuses` и `sync/resync-metadata` переведены на `admin` session access, чтобы ручные mutating routes без cron больше не оставались публичными.
- [x] Manual analysis routes `analysis/order/[id]`, `analysis/insights/run` и `analysis/quality/refresh` переведены на session-based analytics access (`admin|okk|rop`), чтобы тяжёлые insight/aggregate triggers больше не были публичными.
...
- [x] Status backend и status page переведены на fallback-only semantics для legacy Telphin и matching backfill cards: `Telphin Fallback Sync`, `Telphin Backfill Sweep`, `Matching Backfill Sweep` больше не выглядят как primary runtime при активном realtime pipeline.
...
- [x] UI экрана ОКК перестал подсказывать legacy cron ownership: таймер "до проверки" убран, а bulk `run-all` помечен как manual fallback rebuild, чтобы production ownership оставался за realtime pipeline, а не за старым full scan.
...
- [x] Есть feature flag или безопасный способ rollout, если шаг влияет на production pipeline.
- [ ] Есть метрика, по которой можно проверить, что шаг реально дал эффект.
- [x] Есть rollback-путь на случай деградации.
