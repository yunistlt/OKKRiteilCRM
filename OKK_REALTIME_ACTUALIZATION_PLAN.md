...
- [ ] Зафиксировать рабочую нагрузку как baseline: 20-30 заказов в день из RetailCRM.
- [ ] Зафиксировать важное уточнение baseline: одновременно в рабочих статусах остаются около 500 заказов, а не только новые лиды текущего дня.
- [ ] Зафиксировать рабочую нагрузку как baseline: 4 менеджера.
- [ ] Зафиксировать важное уточнение baseline: каждый менеджер в среднем обрабатывает порядка 10 заказов в день, при этом общий активный пул заметно больше дневного потока.
- [ ] Зафиксировать рабочую нагрузку как baseline: около 40 звонков в день на менеджера.
- [ ] Принять расчётную нагрузку для проектирования: до 160 звонков в день суммарно.
- [ ] Принять расчётный средний поток заказов: до 2 заказов в час в среднем и до 5-8 заказов в коротком всплеске.
- [ ] Принять расчётный активный рабочий набор для production-проектирования: около 500 заказов в рабочих статусах, которые нельзя full-scan-ить каждые несколько минут.
- [ ] Принять расчётный средний поток звонков: до 20 звонков в час в среднем и до 15-20 звонков за 10 минут в коротком всплеске.
...
- [ ] Заменить cron-подход "раз в 10 минут по 10 звонков" на непрерывную очередь задач transcription.
- [ ] Обрабатывать звонки по мере появления recording_ready, а не ждать следующего cron-слота.
- [ ] Оставить ограничение concurrency=2 на старте и поднимать только после замера реальной latency.
- [ ] Ввести приоритет транскрибации для звонков по активным рабочим статусам.
- [ ] Ввести приоритет транскрибации для самых свежих звонков, чтобы карточка заказа обновлялась быстро.
- [ ] Для слишком коротких или заведомо нерелевантных звонков ставить skip без отправки в OpenAI.
- [ ] После завершения транскрибации автоматически создавать scoring job по связанному заказу.
- [ ] После завершения транскрибации автоматически запускать semantic rules только для этого звонка или связанного заказа.
...
- [ ] Переделать health-check так, чтобы он смотрел не только на raw_order_events, но и на все критичные очереди.
