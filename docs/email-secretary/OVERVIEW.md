...
> Если код разошёлся с документом — обнови документ. Обновлён: 20.07.2026.
...
- **Переписка по заказу.** Различаем ДВА признака (у них разная сила):
  ...
  - **Голый `Re:` без тега** (`isReplyThread` без `hasCrmOrderTag`) — СЛАБЫЙ (постоянный клиент часто
    ...
    - **`Re:` + номер заказа ЭТОГО клиента в теме** (`findClientOrderNumberInSubject` в крон-роуте) →
      **`reply_thread`** независимо от уверенности ИИ, и письмо сразу привязывается к этому заказу.
      Номер берём как отдельно стоящее число 4–6 цифр из темы и **сверяем с базой**: заказ должен
      существовать И принадлежать тому же email (иначе год/сумма привязали бы письмо к чужому заказу).
      Зачем: менеджеры вписывают номер в тему руками, и тег `[#N/N]` в ветке теряется.
      Инцидент 21.07.2026 — ветка «Re: Запрос КП 53929 …»: три ответа клиента (доп.скидка, обрешётка,
      «по торгам не прошли») завели дубли 53940/53941/53947, ИИ был уверен на 0.9–1.0, порог не спасал;
    - `new_request` без номера в теме → **`reply_thread` ТОЛЬКО если ИИ НЕуверен**
      (`confidence < BARE_RE_NEW_REQUEST_TRUST`, сейчас **0.7**); при уверенной новой заявке ИИ НЕ
      перебиваем — **заводим заказ**. Это чинит инцидент «письмо-запрос КП постоянного клиента уходило
      в переписку вместо новой заявки».
  ...
  - **Привязка `reply_thread` к заказу** — ТОЛЬКО по явному номеру: сверенный номер из темы (см. выше) →
    тег `[#N/N]` / номер из темы или тела
    (`parseOrderNumberFromSubject` → `order_number` от ИИ → regex в теме → regex в теле). Фолбэк «последний
    заказ клиента по email» **убран** — он ложно привязывал новый запрос постоянного клиента к старому
    заказу. Нет явного номера → заказ не привязываем (`created_crm_order_number` = null).
