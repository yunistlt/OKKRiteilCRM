...
- **Переписка по заказу.** Различаем ДВА признака (у них разная сила):
  ...
  - **Голый `Re:` без тега** (`isReplyThread` без `hasCrmOrderTag`) — СЛАБЫЙ (постоянный клиент часто
    ...
    - `new_request` → **`reply_thread` ТОЛЬКО если ИИ НЕуверен** (`confidence < BARE_RE_NEW_REQUEST_TRUST`,
      сейчас **0.7**); при уверенной новой заявке ИИ НЕ перебиваем — **заводим заказ**. Это чинит инцидент
      «письмо-запрос КП постоянного клиента уходило в переписку вместо новой заявки».
  ...
  - **Привязка `reply_thread` к заказу** — ТОЛЬКО по явному номеру: тег `[#N/N]` / номер из темы или тела
