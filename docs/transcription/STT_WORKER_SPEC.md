...
- `mode` (опц., `fresh` | `backlog`) — что отдавать. Без параметра режим выбирается по
  заводскому времени (Тольятти, UTC+4): в ночном окне (по умолчанию 00:00–07:00, ключ
  `sync_state.stt_backlog_window`) отдаётся исторический хвост — только звонки старше двух
  суток, самые старые первыми; в остальное время — только свежие, моложе двух суток.
  Граница жёсткая: днём архив не берётся вовсе, чтобы срочный звонок не ждал за ним в
  очереди. Воркеру ничего менять не нужно: он как ходил в `claim`, так и ходит.

Что попадает в выдачу: звонок с записью, без расшифровки, длиннее порога
`sync_state.transcription_min_duration` (по умолчанию 15 сек). Звонки по заказам, чей статус
помечен «транскрибация» в «Настройке ОКК», уходят первыми — эта пометка задаёт **приоритет**,
а не допуск (до 02.09.2026 она работала как допуск, и очередь из-за этого стояла: заказ уезжал
из рабочего статуса, и его звонки не расшифровывались никогда).
