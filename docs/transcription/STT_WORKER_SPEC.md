...
- `mode` (опц., `fresh` | `backlog`) — что отдавать. Без параметра режим выбирается по
  ...
  `sync_state.stt_backlog_window`) отдаётся исторический хвост — самые старые звонки старше
  двух суток; в остальное время — свежие разговоры, они нужны ОКК и боту-РОПу сегодня.
  Воркеру ничего менять не нужно: он как ходил в `claim`, так и ходит.
