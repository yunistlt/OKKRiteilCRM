...
## Расчётные инструменты (function calling)

Кроме знаний из доков, в глобальной ветке Семён умеет **считать персональную зарплату** через
OpenAI function calling. Инструменты — `lib/salary/consultant-tools.ts`:

- `get_my_salary({ period? })` — расчёт ЗП текущего пользователя за период (источник —
  персистентная строка `salary_calc`, как на странице «Моя зарплата»): итого, оклад, премия,
  K_качества, K_команды, конверсия, ставки и **предельная прибавка за одну заявку**
  (`ставка × K_качества × K_команды`).
- `orders_to_reach({ target_total, period? })` — сколько новых/постоянных заявок закрыть до цели.

Подключаются автоматически, когда у пользователя есть `retail_crm_manager_id`. Только чтение,
зарплатный движок не меняется. **Приватность:** `manager_id` всегда берётся из сессии —
аргументы LLM игнорируются, чужую ЗП получить нельзя. Нет данных → инструмент возвращает
`available:false`, и Семён честно сообщает об отсутствии расчёта.
