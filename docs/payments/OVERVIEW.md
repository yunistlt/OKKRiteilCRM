...
- **Банк — единственный источник правды по оплатам (ЗАКОН).** При пробросе нашего
  банковского платежа любая оплаченная запись на заказе, НЕ созданная синком (нет
  externalId `tochka-`/`tbank-`), сбрасывается в «Не оплачен» (`reconcileOrderPaymentsToBank`),
  чтобы одна и та же оплата не задваивалась (напр. вручную «оплаченный» счёт invoicejur).
