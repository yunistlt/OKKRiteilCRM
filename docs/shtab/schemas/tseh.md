...
Себестоимость в ЦехУспехе — это **расход материалов на заказ**, а не счета
поставщиков: `CostMaterialsItemOrderNDS` берёт количество из
`ItemsNeedMaterialsInOrder` и умножает на цену из счёта поставщика, привязанного
к этому заказу, а если такого счёта нет — на историческую цену закупки за
последние три года (`GetHistoryPriceDPSNDS`). `CostOrderExt` — это отдельная
строка «прочие расходы» по счетам с `TypeCost=4`, у ЗМК почти нулевая; путать её
с себестоимостью нельзя. Инструмент печатает слагаемые отдельно.

Цифры за июль 2026 для сверки глазами с `erp.zmksoft.ru`: выручка без НДС
14 947 686, материалы 4 240 096, зарплата заказов 1 594 815, взносы 446 548,
прочие расходы 5 689, прибыль 8 660 538, маржа 57,9 % — по всем 53 заказам,
материалы посчитаны у всех.
