...
## Глубина истории (проверено на боевой)

- `ListBalanses` — ежедневные снимки с 29.11.2020, 2007 дней, ~68 тыс. строк. Пропусков почти нет.
...
- Нормы времени операций у ЗМК не заполнены, а даты техкарт не отражают реальную длительность
  работы. Производительность считать в деньгах (выпуск за период), не в часах — иначе число
  будет выглядеть убедительно и быть неверным.
