...
// Шагов должно хватать на цепочку «увидел странное — проверил — увидел ещё
// более странное». Пяти хватало на ответ по готовым инструментам и не хватало
// ни на один настоящий разбор: он редко укладывается меньше чем в десяток
// запросов.
//
// Четырнадцати перестало хватать, когда ей открыли всю базу: на «разбери работу
// отдела за сорок дней» все шаги ушли на разведку по ста сорока пяти таблицам,
// и ответа не было вовсе. Разведку теперь закрывает db_schema, а потолок поднят
// — но не до бесконечности: модель, которая на тридцатом запросе ещё не знает,
// что ищет, не найдёт этого и на шестидесятом.
...
export async function runTamara(opts: {
    ...
    return {
        reply:
            'Не смогла собрать ответ: ушла в перебор данных и не уложилась. ' +
            'Спроси уже — назови период и что именно смотреть, так я пойду сразу по делу.',
        usedTools,
        model: lastModel,
    };
}
