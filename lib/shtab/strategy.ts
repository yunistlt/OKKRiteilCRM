...
/**
 * Заготовки проектов из карты ресурсов: по одному проекту на отсутствующий
 * ресурс, в том же порядке очереди, что и в стратегии.
 *
 * Ответственный и срок не выдумываются — их ставит владелец. Пустые поля здесь
 * не недоделка, а то место, где Тамара скажет, что проект без имени и даты
 * остаётся пожеланием.
 */
export function projectDraftsFromResources(resources: readonly ShtabResource[]): string[] {
    return resources
        .map((r) => trimDot(r.missing))
        .filter((title) => title.length > 0)
        .map((title) => `Обеспечить: ${title}`);
}
