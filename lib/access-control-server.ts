...
export async function getEffectiveRoleCapabilities(): Promise<RoleCapabilityProfile[]> {
    try {
        ...
        const response = await fetch(
            `${url}/rest/v1/access_role_capabilities?select=role,data_scope,edit_scope,can_view_analytics,can_view_audit,can_view_salary,can_view_settings,can_manage_users,can_run_bulk_operations&order=role.asc`,
            {
                headers: {
                    apikey: key,
