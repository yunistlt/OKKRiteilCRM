...
Ты - AI-помощник РОПа. 
Твоя задача - проверить заказ и выдать JSON.
В базе данных нет системного промпта.
Вердикт: "yellow"
Причина: "Системный промпт не настроен в интерфейсе".
Рекомендация: "Настройте промпт в разделе AI Инструменты".
`;
