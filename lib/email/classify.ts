...
/**
 * Приглашение к участию в ЭЛЕКТРОННЫХ торгах: участие возможно только через электронную площадку
 * или портал закупок. Компания в таких торгах не участвует — заказ по ним заводить не надо.
 *
 * ВАЖНО, что признак НЕ ловит: внутренние тендеры и закупки заказчика, где ТКП/КП просят прислать
 * на e-mail («закупочная сессия», «запрос предложений», «данная закупка не является торгами»,
 * извещение и ТЗ во вложении) — в них компания участвует, это заявка. Поэтому требуем ОБА условия:
 * письмо про закупочную процедуру И явный признак площадки/портала.
 *
 * На одной модели не строим: gpt-4o-mini по слову «приглашение к участию» уводила в «не заявка»
 * и внутренние тендеры (проверено на письмах СИСЭнергострой и Дорогобуж/Акрон).
 */
export function isElectronicTenderInvite(input: {
    subject?: string | null;
    body?: string | null;
    fromEmail?: string | null;
    fromName?: string | null;
    ...
    const aboutProcedure = /(приглаша|приглашение|к участию|на участие|закупк|тендер|аукцион|конкурс|запрос предложений|запрос котировок|отбор\b|извещени|лот\b|процедур)/.test(text);
    if (!aboutProcedure) return false;

    // Признаки площадки/портала: участие вне почты.
    const platform = [
        /портал закупок/,
        /электронн\w* (торгов\w* )?площадк/,
        /\bэтп\b/,
        /торгов\w* площадк/,
        /личн\w* кабинет/,
        /(зарегистрироват|регистрац\w+|аккредитац\w+)[^.]{0,60}(площадк|портал|систем|сайт)/,
        /(подач\w+|подать|подайте|разместит\w*)[^.]{0,60}(на площадк|на портал|в личном кабинете|на сайте закупк|через (площадк|портал|систем))/,
        /(подтвердит\w*|подтверждени\w*) участия[^.]{0,60}(на площадк|на портал|в кабинете|нажав)/,
        /письмо (создано|сформировано) автоматически/,
        /(44|223)[-\s]?фз/,
        /\bеис\b|zakupki\.gov\.ru/,
        /roseltorg|sberbank-ast|rts-tender|b2b-center|etpgpb|tektorg|fabrikant|otc\.ru|zakupki\.mos\.ru/,
    ].some((re) => re.test(text) || re.test(from));
    if (!platform) return false;

    // Контр-признак: предложение просят прислать письмом — значит участие идёт по почте.
    const byEmail = /(на|по) электронн\w* (почтов\w* )?адрес|направит\w*[^.]{0,40}(на почту|письмом|на e-?mail)|(ткп|кп|предложени\w*)[^.]{0,60}(на e-?mail|на почту|на адрес[^.]{0,30}@)|@[a-z0-9.-]+\.[a-z]{2,}[^.]{0,40}(для|куда)? ?(подач|направлени)/.test(text);
    return !byEmail;
}
...
1) "new_request" — НОВАЯ ЗАЯВКА от клиента: запрос коммерческого предложения (КП), счёта, цены, наличия, расчёта, сроков изготовления/поставки; просьба прислать ФОТО, чертёж, характеристики, сертификат или паспорт КОНКРЕТНОГО изделия (названо изделие, размеры или модель — «пришлите фото шкафа для ЛВЖ 2000х2300х1400»): клиент выбирает НАШУ продукцию, менеджер ответит фото и предложит цену; ТЗ/спецификация на просчёт; приглашение к участию во внутреннем тендере/закупке заказчика, где предложение отправляется НАМ ПО ПОЧТЕ. Любое реальное намерение купить НАШУ продукцию / получить НАШЕ предложение. ИСКЛЮЧЕНИЕ: электронные торги (участие через ЭТП/портал) — см. п.6.
...
2) "accounting" — БУХГАЛТЕРИЯ: ТОЛЬКО вопрос по ДОКУМЕНТАМ и деньгам по УЖЕ ИДУЩЕЙ сделке — подтверждение оплаты, акты сверки, закрывающие документы (акты, накладные, УПД, счёт-фактуры), обмен реквизитами, ЭДО по документам, возврат денежных средств, дебиторка и кредиторка. ВАЖНО: просьба клиента ВЫСТАВИТЬ СЧЁТ на покупку продукции — это НЕ бухгалтерия, а "new_request" (клиент хочет купить). Бухгалтерия начинается там, где счёт уже выставлен.
   ...
   ПОВТОРНАЯ ПОКУПКА — это тоже "new_request". Ссылка на ПРЕЖНИЙ заказ («на поставку по заказу 42519 от 17.12.2024», «как в прошлый раз», «повторить заказ») НЕ делает письмо бухгалтерией: если клиент просит выставить счёт на НОВУЮ поставку/отгрузку (новое количество, новый адрес доставки, новая карточка предприятия/реквизиты) — это НОВАЯ сделка → "new_request". "accounting" — только когда счёт по ЭТОЙ поставке УЖЕ выставлен и вопрос идёт про документы или оплату по нему.
...
- ПРИГЛАШЕНИЕ к участию в закупке, тендере, закупочной сессии, отборе или запросе предложений, где ТКП/КП просят прислать НАМ НА ПОЧТУ → "new_request". Это внутренний тендер заказчика: менеджер отвечает письмом, заявку заводим. Извещение, ТЗ, проект договора во вложении и слова «закупка», «лот», «отбор» этого не меняют.
- И ТОЛЬКО если участвовать можно ИСКЛЮЧИТЕЛЬНО через электронную площадку или портал закупок → "not_request": в электронных торгах компания не участвует. Признаки: нужна регистрация/аккредитация на площадке, подтверждение участия в личном кабинете, подача заявки на портале; письмо от портала закупок («Портал закупок …», «письмо создано автоматически, не отвечайте»); аукцион/конкурс на ЭТП, торги по 44-ФЗ/223-ФЗ, ЕИС; вместо позиций — ссылка на площадку. Если в письме есть позиции и адрес почты для ТКП — это заявка, а не электронные торги.
