...
1) "new_request" — НОВАЯ ЗАЯВКА от клиента: запрос коммерческого предложения (КП), счёта, цены, наличия, расчёта, сроков изготовления/поставки; ТЗ/спецификация на просчёт; приглашение к участию во внутреннем тендере/закупке заказчика, где предложение отправляется НАМ ПО ПОЧТЕ. Любое реальное намерение купить НАШУ продукцию / получить НАШЕ предложение. ИСКЛЮЧЕНИЕ: электронные торги (участие через ЭТП/портал) — см. п.6.
