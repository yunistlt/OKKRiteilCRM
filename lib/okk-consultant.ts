...
const CONSULTANT_SECTION_CONFIGS: ConsultantSectionConfig[] = [
    {
        ...
        topics: [
            ...
            {
                key: 'how-to-improve',
                title: 'Как поднять свой скоринг',
                aliases: [
                    'как поднять скоринг', 'поднять скоринг', 'улучшить скоринг', 'повысить скоринг',
                    'как поднять рейтинг', 'поднять рейтинг', 'повысить рейтинг', 'улучшить рейтинг',
                    'свой скоринг', 'свой рейтинг', 'мой скоринг', 'мой рейтинг',
                    'на что обратить внимание', 'что исправить', 'почему низкий скоринг', 'почему низкий рейтинг',
                    'низкий скоринг', 'низкий рейтинг', 'как улучшить качество', 'поднять балл', 'повысить балл',
                ],
                answer: [
                    'Чтобы поднять скоринг ОКК, нужно работать не с «итоговым процентом», а с конкретными рычагами, из которых он складывается.',
                    '',
                    'Рычаги по порядку отдачи:',
                    '1. Сначала найдите самый слабый блок — Скрипт или Сделка. Обычно сильнее всего проседает Скрипт (качество разговора), и подтянуть его даёт больший прирост, чем добивать и без того высокий блок Сделки.',
                    '2. В слабом блоке смотрите критерии, которые проваливаются чаще всего: например, работа с возражениями, подтверждение бюджета, согласование следующего шага, выявление срочности. Исправление частого критерия поднимает средний балл по всем заказам сразу.',
                    '3. Отдельный рычаг — штрафы за нарушения: они вычитаются из итога напрямую, поэтому их устранение поднимает рейтинг помимо критериев.',
                    '',
                    'Важная оговорка про данные: если по заказу нет записи разговора (звонок не зафиксирован или не привязан), скрипт по нему не оценивается и балл проседает не из-за качества. Поэтому сначала убедитесь, что разговоры фиксируются, и только потом делайте вывод о работе по скрипту.',
                    '',
                    'Персональный разбор: попросите меня «как поднять мой скоринг» — я возьму ваши заказы за период, покажу слабый блок, честно отмечу, где не хватает данных, и дам список критериев, исправление которых даёт наибольший прирост рейтинга.',
                ].join('\n'),
            },
        ],
    },
    ...
    // title per page and pass a subsystem hint; substantive answers come from project_knowledge. ---
    {
        ...
        title: 'Зарплата ОП',
        shortTitle: 'Зарплата',
        aliases: ['зарплата', 'зп', 'оклад', 'премия', 'мотивация', 'выплата', 'бонус'],
        ...
        topics: [
            {
                key: 'client-type',
                title: 'Новый или постоянный клиент',
                aliases: [
                    'новый клиент', 'постоянный клиент', 'повторный клиент', 'тип клиента',
                    'клиент новый', 'клиент постоянный', 'новый или постоянный', 'новый и постоянный',
                    'как определяется клиент', 'кто такой постоянный', 'почему новый', 'почему постоянный',
                    'постоянный или новый', 'повторная продажа', 'повторные продажи',
                ],
                answer: [
                    'В зарплате каждая засчитанная заявка относится к одному из двух типов клиента — новый или постоянный, и от этого зависит ставка премии за заявку.',
                    '',
                    'Как система это определяет:',
                    '1. У каждого заказа берётся клиент из карточки в CRM (компания или контакт, привязанные к заказу).',
                    '2. Считается, сколько заказов этого клиента за всё время дошли до передачи в производство, — то есть сколько у него было реально доведённых до дела сделок, а не просто обращений.',
                    '3. Если таких сделок мало — клиент новый; если их количество превышает порог — клиент постоянный.',
                    '',
                    'Что важно понимать:',
                    '- Учитывается вся история клиента, а не только текущий месяц: одна и та же компания, вернувшаяся за следующим заказом, со временем переходит из новых в постоянные.',
                    '- Текущая сделка тоже входит в счёт, поэтому первый заказ клиента всегда считается новым.',
                    '- Порог, после которого клиент становится постоянным, задаётся в настройках мотивации и может меняться (по умолчанию — больше двух закрытых сделок).',
                    '- Если у заказа в CRM не указан клиент, заявка по умолчанию считается новой.',
                    '',
                    'Практический смысл для менеджера: повторные продажи одному и тому же клиенту система видит и учитывает отдельно, поэтому работа с возвращающимися клиентами отражается в расчёте.',
                ].join('\n'),
            },
            {
                key: 'grades',
                title: 'Грейды менеджера',
                aliases: [
                    'грейд', 'грейды', 'грейдирование', 'ранг', 'разряд', 'уровень менеджера',
                    'грейд менеджера', 'как повысить грейд', 'поднять грейд', 'повышение грейда',
                    'откат грейда', 'понизить грейд', 'грейд коэффициент', 'коэффициент грейда',
                    'что такое грейд', 'как считается грейд', 'как растёт грейд', 'как работает грейд',
                ],
                answer: [
                    'Грейд — это личный ранг менеджера, который работает как повышающий коэффициент к переменной части зарплаты. Шкала: грейд 1 — высший, грейд 3 — низший (стартовый); ниже стартового грейд не опускается.',
                    '',
                    'Как грейд повышается (автоматически):',
                    '1. Каждый закрытый месяц проверяется, выполнил ли менеджер заданные показатели. Месяц «зачтён», если пройдены все обязательные критерии: выполнение личного плана (порог) и положение лучшего в своей роли по конверсии, скорингу качества ОКК и среднему чеку.',
                    '2. За несколько зачтённых месяцев подряд (по умолчанию 3) грейд повышается на один уровень. Ещё столько же зачтённых месяцев подряд — ещё на один.',
                    '3. Сравнение «лучший в отделе» идёт внутри своей роли (продавцы сравниваются с продавцами, операторы — с операторами).',
                    '',
                    'Как грейд откатывается:',
                    '- За несколько месяцев невыполнения подряд (по умолчанию 2) грейд снижается на один уровень, но не ниже стартового (3-го). Один отдельный провал прогресс не обнуляет до тех пор, пока стрик невыполнений не дойдёт до порога.',
                    '',
                    'Как грейд влияет на зарплату:',
                    '- Коэффициент грейда умножает всю переменную часть (премии и переменные бонусы), как коэффициент команды; оклад он не затрагивает.',
                    '- Работает только если в схеме роли подключён блок «Грейд-коэффициент»; в нём для каждого уровня задан свой множитель. Если блок не подключён — грейд на выплату не влияет.',
                    '',
                    'Когда пересчитывается:',
                    '- Грейды пересчитываются каждый месяц после закрытия периода, с настраиваемой глубиной анализа (несколько месяцев назад).',
                    '- Грейд, заработанный по месяцам включая последний закрытый, начинает действовать со следующего месяца. Закрытые расчёты при этом не меняются.',
                    '',
                    'Где настраивается: «Настройки мотивации → Грейды» — там задаются шкала, глубина окна, сколько месяцев подряд нужно для повышения и отката, когорта сравнения и набор показателей. Числа в коде не зашиты.',
                ].join('\n'),
            },
        ],
        // Параметры настройки грейдов — отвечаем точечно на «что такое X» (матчатся раньше тем).
        entities: [
            {
                key: 'grade-cohort',
                title: 'Когорта сравнения (грейды)',
                aliases: ['когорта сравнения', 'когорта', 'с кем сравнивают', 'с кем сравнивается', 'круг сравнения'],
                answer: [
                    '«Когорта сравнения» — это круг менеджеров, с которыми сравнивают данного менеджера в критериях типа «топ отдела» (конверсия, скоринг качества, средний чек) при расчёте грейда.',
                    '',
                    'Доступно два варианта:',
                    '- «Та же роль» — сравнение идёт внутри одной роли: продавцы соревнуются с продавцами, операторы — с операторами. Так сравнение честнее между сопоставимыми позициями (по умолчанию).',
                    '- «Весь реестр» — менеджер сравнивается со всеми участниками реестра ОП сразу, независимо от роли.',
                    '',
                    'Важно: на когорту опираются только критерии «топ отдела». Критерии-пороги (например, выполнение личного плана) считаются по самому менеджеру и от когорты не зависят.',
                ].join('\n'),
            },
            {
                key: 'grade-window',
                title: 'Глубина окна (грейды)',
                aliases: ['глубина окна', 'окно анализа', 'глубина анализа', 'за сколько месяцев смотрит', 'сколько месяцев анализирует'],
                answer: [
                    '«Глубина окна» — сколько последних месяцев система просматривает при пересчёте грейда. Внутри этого окна считается стрик зачтённых и незачтённых месяцев подряд, на основе которого грейд повышается или откатывается.',
                    '',
                    'По умолчанию — 6 месяцев. Окно ограничивает, как далеко назад смотреть; само решение о повышении/откате принимается по непрерывному стрику, заканчивающемуся последним закрытым месяцем.',
                ].join('\n'),
            },
            {
                key: 'grade-streak',
                title: 'Повышение и откат грейда (месяцев подряд)',
                aliases: ['повышение грейда', 'откат грейда', 'понижение грейда', 'месяцев подряд', 'сколько месяцев подряд', 'за сколько повышается грейд'],
                answer: [
                    'Грейд меняется по непрерывным стрикам месяцев:',
                    '- «+1 за N мес. подряд» — сколько зачтённых месяцев подряд нужно, чтобы грейд повысился на один уровень (по умолчанию 3).',
                    '- «−1 за N мес. подряд» — сколько незачтённых месяцев подряд нужно, чтобы грейд откатился на один уровень, но не ниже стартового/низшего (по умолчанию 2).',
                    '',
                    'Один отдельный провал прогресс не обнуляет до уровня грейда — он лишь прерывает стрик выполнений; откат происходит только когда стрик невыполнений дойдёт до своего порога.',
                ].join('\n'),
            },
            {
                key: 'grade-criteria',
                title: 'Критерии выполнения месяца (грейды)',
                aliases: ['критерии выполнения месяца', 'критерии грейда', 'топ отдела', 'обязательный критерий', 'режим порог'],
                answer: [
                    '«Критерии выполнения месяца» — показатели, по которым месяц признаётся зачтённым для грейда. У каждого критерия есть режим:',
                    '- «Порог» — значение менеджера должно быть не меньше (или не больше) заданного. Пример: выполнение личного плана ≥ 100%.',
                    '- «Топ отдела» — менеджер должен войти в топ-N своей когорты сравнения по показателю (конверсия, скоринг качества ОКК, средний чек).',
                    '',
                    'Месяц зачтён, если пройдены все критерии, отмеченные «Обязателен». Набор показателей, режимы и пороги настраиваются в «Настройки мотивации → Грейды».',
                ].join('\n'),
            },
        ],
        // Материализуем в KB, чтобы Семён отвечал про тип клиента и из глобального чата
        // (семантический поиск по эмбеддингам), а не только на странице зарплаты.
        materializeToKb: true,
    },
    {
        // Отчёт бота-РОПа приходит в личку Telegram, страницы у него нет — поэтому
        // pathPrefixes пуст: раздел не привязан к экрану, но живёт в РАГ и отвечает
        // на вопросы «что значит вот эта строка в отчёте» из любого чата.
        key: 'sales-rop',
        title: 'Отчёт по звонкам за день',
        shortTitle: 'Звонки за день',
        aliases: [
            'отчёт по звонкам', 'отчет по звонкам', 'звонки за день', 'разбор дня',
            'бот роп', 'бот-роп', 'вечерний отчёт', 'вечерний отчет', 'сводка по звонкам',
        ],
        summary:
            'Вечерний отчёт бота по звонкам: сколько было звонков, сколько из них подтверждённых разговоров и почему остальные не зачтены.',
        overview: {
            purpose:
                'Отчёт отвечает на один вопрос: сколько за день было настоящих разговоров с клиентами, а не сколько раз поднималась трубка. Счётчик звонков обманывает — сорок звонков выглядят работой, пока в расшифровках половина не окажется гудком или автоответчиком.',
            workflowTitle: 'Как читать отчёт:',
            workflow: [
                'Первая строка — сколько всего звонков было и как они делятся на исходящие и входящие.',
                'Вторая строка — сколько из них подтверждённых разговоров и сколько минут вышло в трубке. Норма считается по разговорам, не по звонкам.',
                'Строка «Не зачтено» разбирает остальные звонки по причинам: автоответчик, без ответа, без записи.',
                'Последняя строка сравнивает день с собственным средним менеджера за две недели.',
            ],
            pitfalls: [
                'Незачтённый звонок не всегда вина менеджера: «без записи» — это пробел в наших данных, а не претензия к человеку.',
                'Нормы по минутам разговора нет намеренно: под план по минутам начинают говорить о погоде, а не о деле.',
            ],
        },
        pathPrefixes: [],
        topics: [],
        entities: [
            {
                key: 'confirmed-talks',
                title: 'Подтверждённых разговоров',
                aliases: [
                    'подтверждённых разговоров', 'подтвержденных разговоров', 'подтверждённый разговор',
                    'что такое разговор', 'что считается разговором', 'реальные разговоры', 'зачтённые звонки',
                ],
                answer: [
                    '«Подтверждённых разговоров» — сколько звонков за день дошли до настоящего диалога с человеком. Это единственная цифра в отчёте, по которой считается норма.',
                    '',
                    'Разговором звонок признаётся только по расшифровке, а не по длительности:',
                    '1. Звонок должен длиться не меньше 15 секунд.',
                    '2. У него должна быть расшифровка.',
                    '3. В расшифровке видны реплики обеих сторон — и менеджера, и клиента. Если расшифровка пришла без разметки ролей, разговором считается текст от 400 символов: сто символов односторонней речи — это приветствие, четыреста — уже разговор.',
                    '',
                    'Всё остальное в разговоры не идёт и выносится в строку «Не зачтено» с указанием причины.',
                ].join('\n'),
            },
            {
                key: 'machine',
                title: 'Автоответчик',
                aliases: [
                    'автоответчик', 'что значит автоответчик', 'голосовое меню', 'робот',
                    'почему автоответчик не зачли', 'ivr',
                ],
                answer: [
                    '«Автоответчик» — звонок попал на робота или голосовое меню, живого собеседника не было.',
                    '',
                    'Определяется по словам в расшифровке, а не по длительности: «Вы позвонили в…», «Вас приветствует…», «Наберите добавочный», «Дождитесь ответа оператора», «Ваш звонок очень важен для нас», «Все операторы заняты», «Абонент временно недоступен», «Нажмите 1».',
                    '',
                    'Почему именно по словам: автоответчик спокойно длится и полторы минуты, по длительности его от разговора не отличить. И слушать автоответчик минуту — не работа: если засчитывать такие звонки, показатель начинает измерять терпение, а не переговоры.',
                ].join('\n'),
            },
            {
                key: 'no-answer',
                title: 'Без ответа',
                aliases: [
                    'без ответа', 'что значит без ответа', 'не дозвонился', 'короткий звонок',
                    'почему без ответа', 'гудки',
                ],
                answer: [
                    '«Без ответа» — трубку не взяли или разговора не получилось.',
                    '',
                    'Сюда попадают две вещи:',
                    '1. Звонок короче 15 секунд — порог берётся из настройки transcription_min_duration и в отчёте не выдуман.',
                    '2. Шум распознавания: на тишине расшифровка выдаёт мусор вроде «Продолжение следует…» или «Субтитры…». Это заставки от обучающих данных, а не речь.',
                    '',
                    'Это нормальная часть работы: обзвон не бывает без недозвонов. Строка нужна, чтобы разница между «сделал сорок звонков» и «поговорил с двенадцатью» была объяснима, а не выглядела придиркой.',
                ].join('\n'),
            },
            {
                key: 'no-record',
                title: 'Без записи',
                aliases: [
                    'без записи', 'что значит без записи', 'почему без записи', 'нет записи',
                    'нет расшифровки', 'звонок без записи', 'запись не пришла',
                ],
                answer: [
                    '«Без записи» — звонок длился 15 секунд и дольше, то есть, скорее всего, был разговором, но расшифровки к нему нет и подтвердить его нечем.',
                    '',
                    'Это не претензия к менеджеру, а пробел в наших данных. Причины две:',
                    '1. Запись не пришла из телефонии — например, плечо звонка не писалось (входящие через очередь и обратный звонок доходят как переадресованные, а такое плечо записывается только при включённой настройке).',
                    '2. Запись есть, но ещё не расшифрована — очередь расшифровки разбирает свежие звонки днём, а старый хвост ночью.',
                    '',
                    'Именно поэтому строка вынесена отдельно от «без ответа» и «автоответчика»: там причина в звонке, а здесь — в нас. В разговоры такой звонок не зачитывается (судить не по чему), но и виной менеджера он не считается.',
                ].join('\n'),
            },
            {
                key: 'two-week-average',
                title: 'Твоё среднее за две недели',
                aliases: [
                    'твоё среднее', 'твое среднее', 'среднее за две недели', 'с чем сравнивают',
                    'больше обычного', 'меньше обычного', 'как обычно',
                ],
                answer: [
                    '«Твоё среднее за две недели» — сравнение менеджера с его собственной базой за предыдущие две недели, а не с коллегами и не с планом отдела.',
                    '',
                    'Показываются три числа: среднее количество звонков, разговоров и минут в трубке за день. Пометка «больше обычного» ставится при отрыве от своего среднего на разговор и выше, «меньше обычного» — при отставании на разговор и ниже, между ними — «как обычно».',
                    '',
                    'Сравнение личное намеренно: у менеджеров разная база клиентов и разный характер сделок, и чужая цифра тут ничего не объясняет. Норма по разговорам задаётся отдельно и с этим средним не связана.',
                ].join('\n'),
            },
            {
                key: 'talk-minutes',
                title: 'Минуты в трубке',
                aliases: [
                    'минуты в трубке', 'в трубке', 'минуты разговора', 'норма по минутам',
                    'почему нет нормы по минутам',
                ],
                answer: [
                    '«В трубке N мин» — суммарная длительность подтверждённых разговоров за день. Это справка, а не норматив.',
                    '',
                    'Нормы по минутам нет намеренно: показатель поощряет длинные разговоры, а не результативные — под план по минутам начинают говорить о погоде. Механика в системе оставлена на случай разбора, но выключена.',
                ].join('\n'),
            },
        ],
        // Материализуем в РАГ: отчёт приходит в Telegram, вопросы по нему задают
        // и там, и Семёну в глобальном чате — ответ должен находиться поиском.
        materializeToKb: true,
    },
    {
        key: 'messenger',
