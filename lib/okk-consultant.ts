...
const CONSULTANT_SECTION_CONFIGS: ConsultantSectionConfig[] = [
    ...
    // title per page and pass a subsystem hint; substantive answers come from project_knowledge. ---
    {
        ...
        topics: [
            ...
            {
                key: 'conversion-denominator',
                title: 'Какие заявки идут в конверсию',
                aliases: [
                    'знаменатель конверсии', 'входящие заявки', 'что идёт в конверсию', 'что считается заявкой',
                    'почему заказ зачтён', 'почему заказ учтён', 'почему заявка учтена', 'заказ зачли в конверсию',
                    'как не считать заявку', 'исключить заявку', 'исключения из конверсии', 'что не считается заявкой',
                    'дубль', 'дубли', 'дубль на тендер', 'дубль заявки', 'дубль не зачёлся', 'дубль не исключился',
                    'как оформить дубль', 'не наша продукция', 'нет таких позиций', 'спам', 'спам-заявка',
                ],
                answer: [
                    'Конверсия — это доля входящих заявок, доведённых до передачи в производство. В знаменатель попадают все заявки месяца, кроме перечисленного ниже. Под каждой заявкой в раскрытии конверсии написано, учтена она или исключена, и почему.',
                    '',
                    'Не считаются заявками совсем:',
                    '- «Спам-заявка» — по статусу заказа.',
                    '- Не наша продукция — статус «Нет таких позиций» либо такая же причина отмены. Запрос на чужой товар не является потерянной продажей, поэтому конверсию не портит.',
                    '',
                    'Дубль на тендер (несколько организаций запрашивают один и тот же тендер) исключается и из знаменателя, и из числителя, но только если оформлен правильно. Нужны три вещи одновременно:',
                    '1. В комментарии указан номер заказа-эталона — того, по которому работаем как по основному. Достаточно написать «дубль 53745».',
                    '2. Хотя бы один товар дубля совпадает с товаром эталона по наименованию и количеству. Суммы совпадать НЕ обязаны: тендер бывает разбит по лотам, доставка идёт отдельной строкой или входит в цену, торговой организации считают без НДС — всё это нормально.',
                    '3. Эталон — либо ещё в работе по тендеру (статусы «Тендер», «Ожидание выхода тендера»), либо уже передан в производство, то есть тендер выигран.',
                    '',
                    'Отменённый дубль тоже засчитывается как дубль: достаточно причины отмены «Отмена: Дубль на тендер», ждать финального статуса не нужно.',
                    'Если эталон сам оформлен дублем, система идёт по цепочке до первоисточника — писать номер самого первого заказа не требуется.',
                    '',
                    'Почему дубль мог остаться учтённым (и что делать):',
                    '- Не указан номер эталона — допишите его в комментарий.',
                    '- Ни один товар не совпал с эталоном по наименованию и количеству — проверьте позиции: количество должно быть тем же. Равнять суммы бессмысленно, система смотрит не на деньги, а на товар.',
                    '- Эталон не в тендерных статусах и в производство не уходил (например, уехал в «Отложено» или в отмену) — тогда дубль остаётся в знаменателе.',
                    '',
                    'Отдельно «Дубль заявки» (одно и то же обращение пришло дважды, например письмом и с сайта): исключается из конверсии целиком, если в статусе «Дубль заявки», в комментарии есть номер существующего заказа и написана причина.',
                    '',
                    'Смысл проверок: статус дубля не должен становиться способом убрать неудобную заявку из конверсии, поэтому фиктивные дубли остаются в знаменателе.',
                ].join('\n'),
            },
        ],
