...
| `lead-defaults.ts` | Дефолты обязательных полей карточки для лидов ботов (коды полей/справочников, «Требуется уточнить», дата следующего контакта, срок изготовления) |
...
## Лид от бота: что заполняем сами

Боты (Катерина — почта, Елена — виджет/чат/корзина, AI-секретарь Телфина — звонок) заводят
заказ через `leads.ts`. Дефолты полей — в `lead-defaults.ts`, один источник на все три бота:

| Поле карточки | Код | Что ставим |
|---|---|---|
| Категория товара | `typ_castomer` | подсказка бота или `trebuetsya-utochnit` |
...
не сохраняет карточку. Коды справочных значений берём из `retailcrm_dictionaries` и только
активные; справочник не прочитался — поле не отправляем (пустое поле лучше битого кода).

Клиент у лида **всегда корпоративный**: `ensureCorporateCustomerId()` ищет контрагента по
ИНН → email → телефону и заводит нового (название компании — распознанное ботом, иначе имя
контакта, иначе «Требуется уточнить»). Если контрагента завести не удалось — откат на физлицо,
лид не теряем.
