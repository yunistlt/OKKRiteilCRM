-- Revert: Link orders to clients

-- 1. Drop the overloaded function with JSONB[] argument
DROP FUNCTION IF EXISTS upsert_orders_v2(jsonb[]);

-- 2. Restore the original function with JSONB argument (just in case it was modified or we need to be sure)
-- This matches the definition from 20260115_fix_upsert_orders_v2_cast.sql
...
-- 3. Drop the client_id column from orders
