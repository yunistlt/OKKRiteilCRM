-- ============================================================================
-- НДС 0% по иностранному контрагенту (экспорт: РБ и др.).
-- Счёт экспортному заказчику выставляется без НДС, но движок ЗП определял ставку
-- ТОЛЬКО по витрине (orders.site) и делил выручку на делитель 5% — выручка и
-- премия занижались (инцидент по заказу 54092, ООО «Оборудованиеторг», Минск).
-- Ставку из карточки позиции использовать нельзя: её массово не проставляют
-- (1756 позиций «0.00» против 67 «5.00» за полгода).
-- Решение: отдаём в расчёт реквизиты контрагента; признаки иностранного
-- контрагента — в конфиге (vat_policy.exempt_contragent_markers), ноль хардкода.
-- Аддитивно: только новая колонка contragent в RETURNS TABLE.
-- Базой взято ЖИВОЕ определение функции из прода (оно новее файлов в migrations/).
-- ============================================================================

DROP FUNCTION IF EXISTS public.salary_counted_orders(timestamp with time zone, timestamp with time zone, text, text, text[], text[], text[], text, text, text[], text[], text[], text[], text[], numeric, boolean);
...
-- Маркеры иностранного контрагента добавляем во ВСЕ версии vat_policy: это
-- исправление ошибки расчёта, оно должно действовать и задним числом (пересчёт
-- прошлых периодов даст корректную выручку без НДС).
UPDATE salary_config
SET value = value || jsonb_build_object(
        'exempt_contragent_markers',
        '["Беларусь","Беларуси","Минск","Минский","УНН","Казахстан","Алматы","Астана","Кыргыз","Бишкек","Узбекистан","Ташкент","Армения","Ереван"]'::jsonb
    )
WHERE key = 'vat_policy';
