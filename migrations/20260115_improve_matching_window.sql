...
BEGIN
    ...
    unmatched_calls AS (
        ...
        -- Removing ORDER BY DESC to potentially pick up older calls if index allows, 
        -- or we should process newest first but ensure backlog is cleared.
        -- Let's keep DESC for now but we might need a separate backfill pass.
    ...
    SELECT 
        ...
        (CASE 
            ...
            WHEN bm.time_diff_sec <= 43200 THEN 'by_phone_day' -- 12 hours (working day)
            ELSE 'by_phone_any'
        END)::TEXT as match_type,
        (CASE 
            WHEN bm.time_diff_sec <= 600 THEN 0.95    -- High
            WHEN bm.time_diff_sec <= 43200 THEN 0.85  -- Medium (same working day)
            ELSE 0.75                                 -- Low
        ...
        (CASE 
            ...
            WHEN bm.time_diff_sec <= 43200 THEN 
                'Совпадение номера, разница ' || ROUND(bm.time_diff_sec / 3600, 1) || ' ч'
            ELSE 
                'Совпадение номера (вне рабочего дня)'
