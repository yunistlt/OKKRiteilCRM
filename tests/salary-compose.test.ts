...
const CFG = {
    oklad: 35000,
    rate_zayavka: { new: 2000, permanent: 1000, pech_vto: 3000 },
...
describe('блочный движок ≡ прежняя формула (пресет «Продавец»)', () => {
    ...
    it('оператор: только оклад 15 000, без переменной части', () => {
        ...
        const m = mkMetrics({ countsByType: { new: 5, permanent: 0, pech_vto: 0 }, qualityAvgScore: 80, conversion: { numerator: 5, denominator: 20, pct: 25, eligible: true } });
