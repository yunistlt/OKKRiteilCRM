...
describe('содержание базы знаний', () => {
    ...
    it('процедура постановки сотрудников на цель покрыта целиком', () => {
        // Выпал шаг — Тамара перестанет о нём спрашивать, и постановка на цель
        // выродится в одно собрание, после которого ничего не меняется.
        const slugs = new Set(SHTAB_KB_SEED.map((r) => r.slug));
        for (const step of [
            'as-postanovka-na-cel',
            'as-sobranie-po-celyam',
            'as-individualnye-vstrechi',
            'as-kaskad-celey',
        ]) {
            expect(slugs, step).toContain(step);
        }
    ...
    it('практикум донесения целей команде покрыт по всем восьми шагам', () => {
        // Выпала статья — Тамара доведёт владельца до написанной цели и
        // отпустит, а именно между написанием и собранием всё и ломается.
        const slugs = new Set(SHTAB_KB_SEED.map((r) => r.slug));
        for (const step of [
            'bp-vosem-shagov',
            'bp-kompas',
