...
async function resolveConfig(asOf: string): Promise<SalaryConfig> {
  ...
  const latest = new Map<string, unknown>(); for (const r of rows) if (!latest.has(r.key)) latest.set(r.key, r.value);
  const cfg = {} as SalaryConfig; for (const key of SALARY_CONFIG_KEYS){ if(!latest.has(key)) throw new Error('missing '+key); (cfg as any)[key]=validateConfigValue(key,latest.get(key)); } return cfg;
}
...
(async () => {
  ...
  const clientDeals=new Map<number,number>(); if(clientIds.length){const d=await sql`select * from salary_client_deal_counts(${sql.array(clientIds)}::bigint[],${closing})`;for(const r of d)clientDeals.set(Number(r.client_id),Number(r.deals));}
  ...
  // plans
  const planRows=await sql`select manager_id,target from salary_plan where year=${YEAR} and month=${MONTH}`;
  let personalPlan:number|null=null, deptPlan:number|null=null;
  for(const p of planRows){ if(p.manager_id==null) deptPlan=Number(p.target); else if(Number(p.manager_id)===MID) personalPlan=Number(p.target); }
  // grade
  const gr=await sql`select grade_level from salary_grade where manager_id=${MID} and effective_from<=${ASOF} order by effective_from desc limit 1`;
  const grade=gr[0]?Number(gr[0].grade_level):null;
  // category names
  const catRows=await sql`select item_code,item_name from retailcrm_dictionaries where entity_type='customField'`;
  const categoryNames:Record<string,string>={}; for(const r of catRows) categoryNames[r.item_code]=r.item_name;

  const ctx={year:YEAR,month:MONTH,businessDays:businessDaysInMonth(YEAR,MONTH),teamRevenueNoVat:metrics.teamRevenueNoVat,personalPlanTarget:personalPlan,departmentPlanTarget:deptPlan,managerGrade:grade,categoryNames};
  const res=computeManagerSalary(m,blocks,ctx as any,SCHEME);
  console.log('=== Матвеева (98) СВЕЖИЙ расчёт июнь 2026 (схема menedzhery, без записи) ===');
  console.log('personalPlan:',personalPlan,'| deptPlan:',deptPlan,'| grade:',grade);
  console.log('premia_zayavki:',res.premiaZayavki,'| total:',res.total,'| oklad:',res.oklad);
  console.log('counts:',JSON.stringify(res.breakdown.counts),'| countsByCategory(metric):',JSON.stringify(m.countsByCategory));
  console.log('countedOrders:',res.breakdown.countedOrders.length,'| new/perm types:',res.breakdown.countedOrders.map(o=>o.type).join(','));
  console.log('blockContributions:');
  for(const c of (res.breakdown.blockContributions||[])) console.log('   ',c.code,'|',(c as any).kind==='multiplier'?'×'+(c as any).multiplier:(c as any).amount,'|',(c as any).explain);
  console.log('teamRevenueNoVat:',Math.round(metrics.teamRevenueNoVat));
  await sql.end();
