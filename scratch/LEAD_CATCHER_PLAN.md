...
### 1.6 Безопасное внедрение постоянного окна консультанта (без риска для текущего сайта)

#### 1.6.1 Изоляция изменений
- [ ] Создать копию текущей темы Webasyst (child theme / дубликат)
- [ ] Все правки макета и стилей делать только в копии темы
- [ ] Боевую тему не переключать до завершения smoke-check

#### 1.6.2 Тестовая витрина (preview)
- [ ] Поднять отдельный settlement/URL для теста (preview)
- [ ] Назначить на preview новую тему с постоянным окном консультанта
- [ ] Проверить ключевые страницы: главная, категория, карточка товара, корзина

#### 1.6.3 Новый макет без перекрытия товаров
- [ ] Внедрить двухколоночный layout на desktop: контент + правая колонка консультанта
- [ ] Убедиться, что консультант не перекрывает карточки и CTA элементов
- [ ] На mobile оставить свёрнутый режим (bottom-sheet/кнопка), чтобы не занимать экран

#### 1.6.4 Сессионная фиксация содержимого окна
- [ ] Зафиксировать сессионный идентификатор диалога при первом открытии
- [ ] Не перезаписывать контекст консультанта при переходах между страницами в рамках сессии
- [ ] Добавить явный сценарий сброса сессии (по кнопке или таймауту)

#### 1.6.5 Управляемый rollout и откат
- [ ] Добавить feature flag: `CONSULTANT_STICKY_LAYOUT_ENABLED`
- [ ] Добавить kill switch для мгновенного возврата к старому overlay-режиму
- [ ] Выпускать поэтапно: карточки товаров → категории → остальные страницы

#### 1.6.6 Контроль перед продом
- [ ] Smoke-check desktop/mobile: видимость, скролл, отсутствие перекрытия товара
- [ ] Проверка стабильности сессии: окно и контекст не сбрасываются при навигации
- [ ] Мониторинг 24-48 часов после релиза: JS errors, конверсия в лиды, обращения в чат

#### 1.6.7 Оценка работ
- [ ] Объём: 6-10 файлов, ~350-700 строк (prod-вариант)
- [ ] Срок: 3-5 рабочих дней с тестированием и поэтапным rollout
